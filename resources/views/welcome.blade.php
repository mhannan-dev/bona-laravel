                        ...
                        <div class="blog-info">
                            ...
                            <ul class="post-footer">
                                <li>
                                    ...
                                    @else
                                        <a href="javascript:void(0);" onclick="document.getElementById('favorite_form{{ $post->id }}').submit();"
                                           class="{{ !Auth::user()->favorite_posts->where('pivot.post_id',$post->id)->count()  == 0 ? 'favorite_posts' : ''}}"><i class="ion-heart"></i>{{ $post->favorite_to_users->count() }}</a>

                                        <form id="favorite_form{{ $post->id }}" method="POST" action="{{ route('post.favorite',$post->id) }}" style="display: none;">
                                            {{ csrf_field() }}
                                        </form>
